...
{
    if ((ui->tabWidget->currentIndex() == 0) && (fOpt->mainFontSize > 0)){
        ...
        info += tr("Resonant circuit frequency range") + ": <br/><span style=\"color:blue;\">" + tfeatures.at(1)
                + "</span><br/>";
        info += tr("Initial magnetic permeability") + " (μ): <span style=\"color:blue;\">" + tfeatures.at(0)
                + "</span><br/>";
        for (int i = 0; i < 39; i++) {
            QString tsise_str = TToroidSize[i];
            QStringList t_size_val = tsise_str.split(",");
...
{
    if ((ui->tabWidget->currentIndex() == 1) && (fOpt->mainFontSize > 0)){
        ...
        QString info = "<p>" + tr("Initial magnetic permeability") + " (μ): <span style=\"color:blue;\">" + ffeatures.at(0)
                + "</span><br/>";
